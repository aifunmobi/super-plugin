...
5. **Re-register hooks** by running `bash <plugin_dir>/install-hooks.sh`. A `git pull` only updates repo files — it does NOT symlink newly added hook files into `~/.claude/hooks` or add their entries to `~/.claude/settings.json`. `install-hooks.sh` is idempotent (adds only what's missing, touches nothing else), so this is safe every update and ensures new hooks (e.g. the SessionStart primer) actually activate. If the script is absent (older checkout predating it), skip this step — the pull will have just added it for next time.
6. **Sync Codex** (if `~/.codex` exists) by running `bash <plugin_dir>/install-codex.sh`. This refreshes the real copies for Codex: the `/super` skill at `~/.codex/skills/super/SKILL.md`, the slash prompt at `~/.codex/prompts/super.md`, and the managed primer block in `~/.codex/AGENTS.md` so the Codex CLI and desktop app stay in sync. Codex scans `~/.codex/skills` and skips symlinked skill/prompt files; it does not scan `~/.claude/skills` directly. Idempotent; no-op if Codex isn't installed. Skip if the script is absent (older checkout).
7. Show the new version number and suggest restarting Claude Code (required for new SessionStart/PreToolUse hooks to load)
8. If already up to date, say so
