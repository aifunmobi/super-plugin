...
6. **Sync Codex** (if `~/.codex` exists) by running `bash <plugin_dir>/install-codex.sh`. This refreshes the symlinked `/super` skill in `~/.codex/skills/super` and the managed primer block in `~/.codex/AGENTS.md` so the Codex CLI and desktop app stay in sync. Idempotent; no-op if Codex isn't installed. Skip if the script is absent (older checkout).
